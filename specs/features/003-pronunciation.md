...
- Activating the control starts playback. Activating it again while it is still speaking
  ...
  playing there.
- Playback uses a voice that matches the text's language. By default the best available
  voice for that language is chosen automatically.
...
- If speech synthesis exists but has no voice for the text's language, the control is
  shown but disabled, and explains on hover that no voice is available for that language.
...
4. When the browser has no speech synthesis, no speaker control appears in the bubble or
   the side panel.
5. When no voice exists for the text's language, the control is present but disabled and
   communicates why on hover.
